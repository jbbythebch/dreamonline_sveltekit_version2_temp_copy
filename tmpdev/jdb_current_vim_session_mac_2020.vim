...
inoremap <F4> =strftime("%Y-%m-%d %a %I:%M %p")
inoremap <F3> =strftime("%Y-%m-%d")
...
nnoremap ,e :%s/\s\+$//gc
nnoremap ,T :r /Volumes/Documents1/JAC/templates/vim\ templates/My\ Project,\ To\ Do,\ current\ task\ notes.txt
noremap ,sf :execute "vimgrep /" . expand("<cword>") . "/j " .input("which folders & file extension? ") | cw
noremap ,sg :execute "vimgrep /" . expand("<cword>") . "/j **/" .input("file name & extension? ") | cw
map ,l :call LongLineHighlightToggle()
map <silent> ,h :syntax sync fromstart
...
nnoremap <F4> a=strftime("%Y-%m-%d %a %I:%M %p")
nnoremap <F3> a=strftime("%Y-%m-%d")
...
map <F6> :tabs
map <F5> :ls
map <C-D> :execute 'NERDTreeToggle' .getcwd()
...
set fillchars=vert:|,fold:-,eob:~,eob:\ 
...
set runtimepath+=~/.vim/bundle/coc-svelte
...
set runtimepath+=~/.vim/bundle/vim-snippets-jdb
set runtimepath+=/Applications/MacPorts/MacVim.app/Contents/Resources/vim/vimfiles
set runtimepath+=/Applications/MacPorts/MacVim.app/Contents/Resources/vim/runtime
set runtimepath+=/Applications/MacPorts/MacVim.app/Contents/Resources/vim/runtime/pack/dist/opt/matchit
set runtimepath+=/Applications/MacPorts/MacVim.app/Contents/Resources/vim/vimfiles/after
...
set runtimepath+=~/.vim/bundle/vim-svelte/after
set runtimepath+=~/.vim/bundle/coc-svelte/after
...
set runtimepath+=~/.vim/bundle/vim-snippets-jdb/after
set shiftwidth=2
...
set smarttab
set softtabstop=2
set statusline=%{coc#status()}%{get(b:,'coc_current_function','')}%f\ \ %m%=▽%l\ ▷%c\ ▼%L\ b-%n%#error#%{&paste?'[paste]':''}%*
set suffixes=.bak,~,.o,.h,.info,.swp,.obj,.snap
set tabstop=2
set updatetime=300
set wildignore=*.png,*.jpg,*.svg,*.log,tmp/**
set nowritebackup
let s:so_save = &g:so | let s:siso_save = &g:siso | setg so=0 siso=0 | setl so=-1 siso=-1
let v:this_session=expand("<sfile>:p")
silent only
silent tabonly
cd /Volumes/Documents1/DreamOnline/apps_front_end/dreamonline_sveltekit
...
argglobal
%argdel
$argadd src/routes/_all-locales/entry-create/+page.svelte
$argadd src/routes/_all-locales/user-register/+page.svelte
$argadd src/routes/_all-locales/user-sign-in/+page.svelte
$argadd src/routes/_all-locales/user-sign-out/+page.svelte
set stal=2
tabnew
tabnew
tabnew
...
tabnew
tabrewind
edit src/lib/graphql/queries/entries-all.js
...
argglobal
3argu
if bufexists("src/lib/graphql/queries/entries-all.js") | buffer src/lib/graphql/queries/entries-all.js | else | edit src/lib/graphql/queries/entries-all.js | endif
balt src/routes/_all-locales/entries-all/+page.svelte
setlocal keymap=
setlocal noarabic
setlocal autoindent
setlocal backupcopy=
...
silent! normal! zE
let &fdl = &fdl
let s:l = 39 - ((0 * winheight(0) + 32) / 64)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 39
...
silent! normal! zE
let &fdl = &fdl
let s:l = 70 - ((63 * winheight(0) + 32) / 64)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 70
...
wincmd w
argglobal
if bufexists("src/lib/client-utils/get-user-email.js") | buffer src/lib/client-utils/get-user-email.js | else | edit src/lib/client-utils/get-user-email.js | endif
balt src/lib/client-utils/init-client.js
setlocal keymap=
setlocal noarabic
setlocal autoindent
...
exe 'vert 5resize ' . ((&columns * 89 + 135) / 270)
tabnext
edit src/routes/_all-locales/entries-all/+page.svelte
set splitbelow splitright
wincmd _ | wincmd |
split
1wincmd k
...
wincmd w
argglobal
if bufexists("src/routes/en/entries-all/+page.svelte") | buffer src/routes/en/entries-all/+page.svelte | else | edit src/routes/en/entries-all/+page.svelte | endif
balt src/routes/_all-locales/entries-all/+page.svelte
setlocal keymap=
setlocal noarabic
setlocal noautoindent
